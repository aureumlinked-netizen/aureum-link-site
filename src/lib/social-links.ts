 ...
 *
 * `fallbackVideoId` — последний рубеж: он нужен, только если `/api/live` вообще
 * не ответил (локальная сборка без Cloudflare Functions) или если у функции не
 * сработал ни один из трёх источников. В обычной работе ID приходит с сервера,
 * и правки в коде для смены эфира не нужны.
 */
